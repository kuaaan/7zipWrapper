...
{
    xErrorLog(L"[7z.dll][GetMethodProperty]");
    xErrorLog(L"\t[IN] codecIndex : %d", codecIndex);
    xErrorLog(L"\t[IN] propID : 0n%d", propID);
  ...
  xErrorLog(L"\t[OUT] PROPVARIANT *value ");
...
{
    xErrorLog(L"[7z.dll][GetNumberOfMethods]");
    xErrorLog(L"\t[OUT] numCodecs : %d", g_NumCodecs);
...
{
    xErrorLog(L"[7z.dll][GetMethodProperty]");
    xErrorLog(L"\t[OUT] IHashers **hashers ");
