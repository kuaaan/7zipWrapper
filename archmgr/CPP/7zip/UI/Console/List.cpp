  ...
  */
  FOR_VECTOR (i, _fields)
  {
    ...
    if (f.IsRawProp)
    {
      ...
      //Item.FileSize.QuadPart = dataSize;
...
{
    PARCH_RESULT pResult = CxArchMgr::GetResultBuffer();
    if (NULL == pResult)
    {
        return S_OK;
    }
  ...
  FOR_VECTOR (r, arcLink.Arcs)
  {
    ...
    pResult->ArchPath = arc.Path;
    pResult->ArchFileSize.QuadPart = arc.FileSize;
    ...
    pResult->ArchType = codecs->GetFormatNamePtr(arc.FormatIndex);
    ...
    UInt64 &numWarnings)
{
  ...
  PARCH_RESULT      pResult = CxArchMgr::GetResultBuffer();
  if (NULL == pResult || NULL == pResult->pItemList)
  {
      return S_OK;
  }
